...
def create_background_image(image_path):
    bpy.ops.mesh.primitive_plane_add(size=50, location=(0, 20, 0))
...
def animate_fade_move_reappear(obj, start_location, end_location, start_frame, fade_frame, reappear_frame, end_frame):
    ...
    bsdf.inputs['Alpha'].keyframe_insert('default_value', frame=fade_frame)

    # Ending position of the object (move to another location within the camera's view)
    obj.location = start_location  # Move to the opposite side within the camera's view
    obj.keyframe_insert(data_path="location", frame=fade_frame)
    
    # Ending position of the object (move to another location within the camera's view)
    obj.location = end_location  # Move to the opposite side within the camera's view
    obj.keyframe_insert(data_path="location", frame=reappear_frame)


    # Animate alpha value of the Principled BSDF shader from 1 to 0
    bsdf.inputs['Alpha'].default_value = 0.0  # Fully opaque
    bsdf.inputs['Alpha'].keyframe_insert('default_value', frame=reappear_frame)
    ...
    bpy.context.scene.frame_end = end_frame + 30
...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\teleporting4_front_id_{i+1}.mp4"  # Unique filename for each video
        ...
        obj1 = create_random_object_basic_set2()
        obj2 = create_random_object_basic_set2()
        obj3 = create_random_object_basic_set2()
        obj4 = create_random_object_basic_set2()        
        ...
        animate_fade_move_reappear(obj1, start_location=(-8, -8, 1), end_location=(-8, 8, 1), start_frame=10, fade_frame=40, reappear_frame=60, end_frame=90)
        animate_fade_move_reappear(obj2, start_location=(-8, 8, 1), end_location=(8, 8, 1), start_frame=10, fade_frame=40, reappear_frame=60, end_frame=90)
        animate_fade_move_reappear(obj3, start_location=(8, 8, 1), end_location=(8, -8, 1), start_frame=10, fade_frame=40, reappear_frame=60, end_frame=90)
        animate_fade_move_reappear(obj4, start_location=(8, -8, 1), end_location=(-8, -8, 1), start_frame=10, fade_frame=40, reappear_frame=60, end_frame=90)

        # Render the animation
        bpy.ops.render.render(animation=True)

# Assuming Windows path, adjust as needed
base_path = "C:\\synta\\teleporting"
