...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\not_colliding3_front_id_{i+1}.mp4"  # Unique filename for each video
        ...
        # Define the animation path waypoints and frames
        # Start locations are off-screen, mid locations are close but not colliding, end locations are off-screen again
        waypoints_obj1 = [(-20, -2, 6), (-2, -2, 6), (20, -2, 6)]
        waypoints_obj2 = [(20, 2,3), (2, 2, 3), (-20, 2, 3)]
        waypoints_obj3 = [(30, 20, 0), (-10, 5, 0), (-20, -20, 0)]
        waypoints_obj4 = [(-20, -25, -3), (10, -5, -3), (40, 5, -3)]
        frames = [1, 60, 120]  # Start, mid (close approach), end frames

        # Create and animate the first moving object
        moving_obj1 = create_random_object_set1()
        animate_object(moving_obj1, waypoints_obj1, frames)

        # Create and animate the second moving object
        moving_obj2 = create_random_object_set1()
        animate_object(moving_obj2, waypoints_obj2, frames)

        # Create and animate the third moving object
        moving_obj3 = create_random_object_set1()
        animate_object(moving_obj3, waypoints_obj3, frames)

        # Create and animate the third moving object
        moving_obj4 = create_random_object_set1()
        animate_object(moving_obj4, waypoints_obj4, frames)
