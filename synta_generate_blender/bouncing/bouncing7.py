...
def setup_camera(target):
    ...
    z = random.uniform(20, 40)  # Adjust the Z to change the height of the camera
...
def animate_bounce_series(obj, start_pos, num_bounces, y_decrease, z_decrease, start_frame, frames_per_bounce):
    """
    Animate an object to bounce multiple times with decreasing height.
    ...
    - y_decrease: Decrease in y for each bounce.
    - z_decrease: Decrease in z height after each bounce.
    ...
    for bounce in range(num_bounces):
        ...
        peak_pos = (current_pos[0], current_pos[1] - y_decrease / 2, current_pos[2] + z_decrease)
        obj.location = peak_pos
        obj.keyframe_insert(data_path="location", frame=peak_frame)
        
        # End of the bounce
        current_frame += frames_per_bounce
        current_pos[1] -= y_decrease  # Decrease y coordinate
        current_pos[2] -= z_decrease  # Decrease z height
        obj.location = current_pos
        obj.keyframe_insert(data_path="location", frame=current_frame)
...
def generate_videos(base_path, number_of_videos=100):
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\bouncing7_id_{i + 1}.mp4"  # Unique filename for each video
