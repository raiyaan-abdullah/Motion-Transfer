...
def animate_object(obj):
    ...
    directions = [(5, 0, 0), (0, 5, 0), (-5, 0, 0), (0, -5, 0)]  # Directions for each clone's movement

    # Adjust the loop to include an additional clone, making a total of 4 moves (including the original)
    for i in range(5):  # Increase range to 5 to create 4 clones plus the original
        if i == 0:
            clone = obj  # The first "clone" is the original object
        else:
        ...
        clone_start_frame = initial_start_frame + (clone_movement_duration + time_between_clones)
        ...
        # Move the clone
        if i > 0:  # Skip the original object for movement
...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\cloning4_front_id_{i+1}.mp4"  # Unique filename for each video
