...
def animate_object(obj):
    start_frame = 1
    mid_frame = 60  # Midpoint where the object reaches the other side
    end_frame = 120  # End of the animation, object returns to the original position

    # Initial position of the object (within camera view)
    initial_position = (-5, 0, 1)  # Starting position
    obj.location = initial_position
    obj.keyframe_insert(data_path="location", frame=start_frame)
    
    # Midpoint position of the object (other side within the camera's view)
    midpoint_position = (5, 0, 1)  # Destination
    obj.location = midpoint_position
    obj.keyframe_insert(data_path="location", frame=mid_frame)

    # Ending position of the object (back to the original position)
    obj.location = initial_position  # Return to start
    obj.keyframe_insert(data_path="location", frame=end_frame)

    # Set the scene to end at the end_frame to complete the animation cycle
...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\moving_in_a_path3_front_id_{i+1}.mp4"
