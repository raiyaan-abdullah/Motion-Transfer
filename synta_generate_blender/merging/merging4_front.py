...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        output_file_path = f"{base_path}\\merging4_front_id_{i+1}.mp4"
        ...
        obj1, shape1 = create_random_object_basic_set2(color)
        obj1.location = locations[0]
        objects1 = [obj1]  # Start a list of objects with the first object
        objects2 = []

        # Loop to create and position additional objects
        for id, loc in enumerate(locations[1:]):
            while True:
                obj, shape = create_random_object_basic_set2(color)
                if shape == shape1:
                    obj.location = loc  # Position it only if the shape matches
                    if id <=0:
                        objects1.append(obj)  # Keep track of the created object
                    else:
                        objects2.append(obj)  # Keep track of the created object
        ...
        animate_objects_to_center(objects1, start_frame, merge_frame, merge_location = (0, 5, 1))
        animate_objects_to_center(objects2, start_frame, merge_frame, merge_location = (0, -5, 1))
        ...
        scale_object(objects1[0], scale_factor, scale_frame, hold_duration)
        scale_object(objects2[0], scale_factor, scale_frame, hold_duration)
        ...
        # Delete the other two objects, keeping only obj1 (now scaled and representing the merged object)
        handler_func = setup_delete_handler([objects1[1], objects2[1]], delete_after_frame)
