...
def generate_videos(base_path, number_of_videos=50):
    ...
    # Number of objects to apply the fade-out animation to
    num_fade_out_objects = 2    
    
    for i in range(number_of_videos):
        # Setup scene for each video
        output_file_path = f"{base_path}/appearing4_front_id_{i+1}.mp4"
        ...
        # Adjust the total animation frames if needed
        # For this scenario, it could be based on just the fading objects
        total_animation_frames = animation_length + (num_fade_out_objects - 1) * (animation_length + delay_between_fades)
        bpy.context.scene.frame_end = total_animation_frames + 20

        start_frame = 1
        
        # Create objects at specific locations
        for location_index, location in enumerate(locations):
            obj = create_random_object_basic_set2()
            # Set each object's location
            obj.location = location

            # Apply the fade-out animation only to the first two objects
            if location_index < num_fade_out_objects:
                ...
                animate_fade_in(obj, start_frame, end_frame)

                # Update start_frame for the next object to fade out
