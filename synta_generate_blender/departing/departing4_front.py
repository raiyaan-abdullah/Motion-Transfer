...
# Main function to generate videos with three stationary and one moving object
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\departing4_front_id_{i+1}.mp4"  # Unique filename for each video
        ...
        # Define locations for the stationary objects
        static_obj_locations = [
            (-4, 4, 1),  # Position for the first stationary object
            (-4, -4, 1), # Position for the second stationary object
            (4, 4, 1)   # Position for the third stationary object
        ]
        ...
        # Object that will animate into, through, and out of the scene
        # Ensure the moving object's path does not intersect with the stationary objects
        start_location = (4, -4, 1)
        stay_location = (4, -40, 1)  # Mid-point where the object will slow or pause

        
        # Timing for the moving object's animation: enter, pause, and exit
        start_frame = 20
        end_frame = 60
        ...
        animate_object(moving_obj, start_location, stay_location, start_frame, end_frame)
        ...
        bpy.context.scene.frame_end = end_frame + 30  # Add buffer to ensure full exit is captured
