...
def animate_object(obj, central_obj):
    start_frame = 1
    end_frame = 120  # Duration of the animation in frames
    radius = 5  # Distance from central object to orbiting object
    half_frame = end_frame // 2  # Midpoint of the animation

    for frame in range(start_frame, end_frame + 1):
        if frame <= half_frame:
            # Clockwise for the first half
            angle = 2 * math.pi * ((frame - start_frame) / (half_frame - start_frame))
            x = central_obj.location.x + radius * math.cos(angle)
            y = central_obj.location.y - radius * math.sin(angle)
        else:
            # Counter-clockwise for the second half
            angle = 2 * math.pi * ((frame - half_frame) / (end_frame - half_frame))
            x = central_obj.location.x + radius * math.cos(angle)
            y = central_obj.location.y + radius * math.sin(angle)
...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        output_file_path = f"{base_path}\\orbiting3_front_id_{i+1}.mp4"  # Adjust path as necessary
