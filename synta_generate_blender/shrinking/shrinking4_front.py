...
def animate_object(obj, scale_factor):
    start_frame = 1
    end_frame = 60

    # Set initial location and scale keyframes at the start frame
    obj.location.y = 0
    obj.scale = (3, 3, 3)  # Starting scale
    obj.keyframe_insert(data_path="location", frame=start_frame)
    obj.keyframe_insert(data_path="scale", frame=start_frame)
    
    # Set final location (if needed) and scale keyframes at the end frame
    obj.scale = (scale_factor, scale_factor, scale_factor)  # Ending scale, now based on the parameter
    obj.location = (obj.location.x, obj.location.y, 1)
...
def generate_videos(base_path, number_of_videos=50):
    ...
    for i in range(number_of_videos):
        ...
        output_file_path = f"{base_path}\\shrinking4_front_id_{i+1}.mp4"  # Unique filename for each video
        ...
        # Create two random objects with glistening material
        obj1 = create_random_object_set2()
        obj2 = create_random_object_set2()

        # Adjust the positions of obj1 and obj2 so they don't overlap
        obj1.location = (-4, -4, 3)  # Move obj1 to the left
        obj2.location = (6, 6, 3)   # Move obj2 to the right

        # Setup camera to focus on the objects
        target = bpy.data.objects.new("Target", None)  # Create an empty object to serve as a target
        bpy.context.collection.objects.link(target)
        target.location = (0, 0, 1)  # Focus between the two objects
        setup_camera(target)

        # Animate both objects with different scale factors
        animate_object(obj1, 0.8)  # Object 1 shrinks more
        animate_object(obj2, 2)  # Object 2 shrinks less
