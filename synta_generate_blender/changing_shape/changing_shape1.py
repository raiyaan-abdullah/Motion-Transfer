...
def setup_initial_object_and_sphere(initial_obj, color, location=(0, 0, 1)):
    # Make sure the initial object is not affecting the sphere's creation
    bpy.ops.mesh.primitive_uv_sphere_add(radius=1, location=location)
    sphere = bpy.context.object
    sphere.name = 'TransformingSphere'
    sphere.scale = (0.001, 0.001, 0.001)  # Start with a visible but small size

    # Assign a visible material
    glossy_mix_group = create_glossy_mix_node_group(color)
    ...
    return initial_obj, sphere


def animate_transformation(initial_obj, sphere, start_frame, end_frame):
    ...
    sphere.scale = (0.1, 0.1, 0.1)  # Make sure the sphere is visible from the start
    ...
    # Ensure the sphere grows to a noticeable size by the end
    sphere.scale = (1, 1, 1)
...
def generate_videos(base_path, number_of_videos=100):
    for i in range(number_of_videos):
        ...
        while obj_type == "SPHERE" or obj_type == "TORUS": #the hollow center of Torus does not create good animation
        ...
        # Setup the sphere alongside the initial object
        initial_obj, sphere = setup_initial_object_and_sphere(initial_obj, color, initial_obj.location)
        ...
        animate_transformation(initial_obj, sphere, start_frame=1, end_frame=60)
