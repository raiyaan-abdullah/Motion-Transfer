...
def setup_initial_object_and_pyramid(initial_obj, color, location=(0, 0, 1)):
    # Make sure the initial object is not affecting the pyramid's creation
    bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=1, depth=2, location=(0, 0, 1))
    pyramid = bpy.context.object
    pyramid.name = 'TransformingPyramid'
    pyramid.scale = (0.001, 0.001, 0.001)  # Start with a visible but small size
    ...
    return initial_obj, pyramid


def animate_transformation(initial_obj, pyramid, start_frame, end_frame):
    ...
    pyramid.scale = (0.2, 0.2, 0.2)  # Make sure the pyramid is visible from the start
    pyramid.keyframe_insert(data_path="scale", frame=start_frame)
    ...
    # Ensure the pyramid grows to a noticeable size by the end
    pyramid.scale = (1, 1, 1)
    pyramid.keyframe_insert(data_path="scale", frame=end_frame)
...
def generate_videos(base_path, number_of_videos=100):
    for i in range(number_of_videos):
        output_file_path = f"{base_path}\\changing_shape3_id_{i+1}.mp4"
        ...
        while obj_type == "PYRAMID" or obj_type == "TORUS": #the hollow center of Torus does not create good animation
        ...
        # Setup the pyramid alongside the initial object
        initial_obj, pyramid = setup_initial_object_and_pyramid(initial_obj, color, initial_obj.location)
        ...
        animate_transformation(initial_obj, pyramid, start_frame=1, end_frame=60)
